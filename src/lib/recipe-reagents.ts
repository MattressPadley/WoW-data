...
export async function resolveRecipeReagents(api: WoWAPI, recipeId: number, noCache: boolean): Promise<ResolvedRecipe> {
  ...
  if (srRow) {
    ...
    return { type: "old", recipe_id: recipeId, recipe_name: recipe.name, spell_id: parseInt(spellId, 10), reagents };
  }

  // Path 2: Modified crafting (Dragonflight+)
  ...
  const reagentSlots = await Promise.all(
  ...
  return { type: "modern", recipe_id: recipeId, recipe_name: recipe.name, spell_id: parseInt(spellId, 10), reagent_slots: reagentSlots };
}
